...
if(isset($_GET['cancel'])){
	...
	$total_pro=$_SESSION['total_pro']; $cus_id=$_SESSION['log_id'];
	
	$sel=" select * from orders "; $res=$db->select($sel); $max=$_SESSION['last_order_id']; $min=$max-$total_pro+1;  
	//echo $min."/$max";
	
	$del=" delete from orders where  Cus_id='$cus_id'  AND  ID BETWEEN $min AND $max ";
	$run=$db->delete($del);
	
	if($run == true) { $_SESSION['cancel']="Order Cancelled !"; unset($_SESSION['order']); }
}
...
if(isset($_GET['id'])){
	...
	$res=$db->update($del);
	if($res===true) { $_SESSION['del']='Item removed from cart';   }
}

$cart="select * from cart where Ses_id='$id' "; 
$cart_no=$db->select($cart)->num_rows; if($cart_no==0) $_SESSION['cart_no']=0; else $_SESSION['cart_no']=$cart_no;
...
<body>
	...
	<!-- Menu -->
    	...
    	<div class="cartoption">		
			<div class="cartpage">    
	<h2 class="w-100 ">Your Cart <small class="ml-5 text-success bg-light"><?php Session::show('del'); Session::show('cancel');
          	Session::show('update'); Session::show('addcart'); Session::show('order');   ?></small>
	        <?php if(isset($_SESSION['order'])) { ?> <a href="cart.php?cancel=cancel" 
	class="btn btn-danger text-light float-right pb-1" onclick="return confirm('Are you sure to cancel?')">Cancel Previous Order</a> <?php } ?>  </h2> 
						...
						<table class="tblone">
							...
							<?php   $sel="select * from cart where Ses_id='$id'"; $res=$db->select($sel); $tot=0; $c=0; $tot_q=0;
				           ...
				           if($res->num_rows> 0){  while($row=$res->fetch_assoc() ) {  							 ?>
							...
							<tr> <?php  if($res->num_rows== 0) ?>
								...
								<td>
									<form action="cart.php?pro_id=<?php echo $row['Cart_id'];  ?>" method="post">
								<input type="number" min="1" max="10" name="new_quantity" value="<?php echo $row['Quantity']; $q=$row['Quantity']; ?>"/>
										<input type="submit" name="update" value="Update"/>
									</form>
								</td>
								<td>$<?php echo $total=$row['Price']*$row['Quantity'];  ?></td>
								<td><a onclick="return confirm('This item will be deleted');"href="cart.php?id=<?php echo $row['Cart_id'];  ?>">X</a></td>
							</tr>
							
							
							 <?php  $tot=$tot+$total; $tot_q=$tot_q+$q; } $_SESSION['q']=$tot_q;  $_SESSION['cart_no']=$c;} 
							 else echo "<tr><h3 class='text-center text-success'>Cart is empty !</h3></tr>";   ?>
							
						</table>
						
						<table style="float:right;text-align:left; margin-top:18px" width="40%">
							<tr>
								<th>Sub Total : </th>
								<td><?php if($c!=0)   echo '$'.$tot; ?></td>
							</tr>
							<tr>
								<th>VAT : </th>
								<td><?php if($c!=0) echo '10%=($'.$vat=$tot*.1,')';  ?></td>
							</tr>
							<tr>
								<th>Grand Total :</th>
								<td> <?php if($c!=0) { echo '$'.($tot+$vat);  $payment=($tot+$vat);} ?> </td>
							</tr>
					   </table>
					</div>
					<div class="shopping">
						...
						<div class="shopright">
							<a <?php if($cart_no==0) { ?> onclick ="cart();" href="" <?php  } elseif 
							(!isset($_SESSION['login']) && !isset($_COOKIE['log_cookie']) ) { ?> onclick ="login();" href="" <?php }   else { ?>
							href="payments.php?total=<?php echo $payment; ?>"  <?php }  ?> > <img src="images/check.png" alt="" /></a>
