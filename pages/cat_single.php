...
$sel="select * from products order by ID DESC"; $pro=$db->select($sel); 
$sel2="select * from category order by ID DESC"; $cat=$db->select($sel2); 
...
<?php  while($category=$cat->fetch_assoc()){ if($cat_id==$category['ID']) { ?>
		<h2 class="my-5 py-1 bg-light text-dark w-100 text-center font-italic font-weight-bold">
		Products under the category: <?php echo $category['catName']; ?></h2>
	...
	<?php  } } ?>
        ...
        <div class="w-100">
		 ...
		 <div style="width:100%" class="section group float-left">
				...
				while($row=$pro->fetch_assoc() ) { if($cat_id==$row['Cat_id']) { ?>
				
				
				<div class="col-sm-3 sec1 <?php if($cnt==4) { ?> mb-5 <?php } ?>">
				...
				<?php $cnt++;   } }  if($cnt==0) echo "<h4 class=' w-100 text-center' >"."This Category has no products to show !</h4>"; ?>
